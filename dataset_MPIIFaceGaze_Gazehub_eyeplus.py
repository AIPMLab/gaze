...
class MPIIFaceGaze_Gazehub_Dataset(Dataset):
    def __init__(
        self,
        root,
        T_sample: int = 32,
        ...
        if self.enableINFO:
            print("[Data] Using dataset: MPIIFaceGaze-preprocess by Gazehub")
            print(f"[Data] T_sample: {self.T_sample}")

        label_files = sorted([f for f in os.listdir(self.label_root) if f.endswith(".label")])
        
        if len(label_files) == 0:
            raise RuntimeError(f"[ERROR] No .label files found in {self.label_root}")
        ...
        for lf in label_files:
            ...
            for line in lines:
                ...
                parsed.append({
                    ...
                    "origin_str": origin_str,  # MPIIFaceGaze独有
                    "which_eye": which_eye,    # MPIIFaceGaze独有

                    "gaze3d": torch.tensor(parse_floats(cols[5]), dtype=torch.float32),
                    "head3d": torch.tensor(parse_floats(cols[6]), dtype=torch.float32),
                    "gaze2d": torch.tensor(parse_floats(cols[7]), dtype=torch.float32),
                    "head2d": torch.tensor(parse_floats(cols[8]), dtype=torch.float32),
                    "rmat": torch.tensor(parse_floats(cols[9]), dtype=torch.float32),
                    "smat": torch.tensor(parse_floats(cols[10]), dtype=torch.float32),
                    "origin":torch.tensor(parse_floats(cols[11]), dtype=torch.float32),
                })

            # sort parsed by numeric frame id from face_path's filename
            def frame_id(p):
            ...
            # store lists (do not load images here)
            face_paths = [e["face_path"] for e in parsed]
            
            labels = {
                k: torch.stack([e[k] for e in parsed], dim=0)
                for k in ["gaze3d", "gaze2d", "head3d", "head2d", "rmat", "smat", "origin"]
            }
...
if __name__ == "__main__":
    print("[INFO] Checking dataset: MPIIFaceGaze preprocessed by gazehub")
    dataset_test = MPIIFaceGaze_Gazehub_Dataset(
        root="/home/NWE/miniconda3/envs/worksacpe_gaze/dataset/MPIIFaceGaze-gazehub",
        T_sample = 4,
        stride = 1,
        img_resize=(256, 256),
        eye_resize=(60, 36),
